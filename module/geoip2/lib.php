...
function giGetCountry($ip){
    ...
    if (empty($ip) || !is_string($ip) || strpos($ip, '.') === false) return '';
    $ip_array = explode('.', $ip);
    $ip0 = isset($ip_array[0]) ? (int)$ip_array[0] : 0;
    $ip1 = isset($ip_array[1]) ? (int)$ip_array[1] : 0;

    fseek($fh, ($ip0 * 256 + $ip1) * 4);
...
View::setPage('client_ip', $_GS['client_ip']);
