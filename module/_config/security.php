...
if (strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET')) === 'POST')
{
	try
	{
		...
		if ($action === 'scan')
		{
			...
			addMsg(($state['status'] ?? '') === 'completed'
				? cfg_t('Проверка целостности завершена.', 'Integrity scan completed.')
				: cfg_t('Проверка начата. Продолжите следующий ограниченный пакет.', 'Scan started. Continue with the next bounded batch.'));
		}
		elseif ($action === 'continue')
		{
			...
			addMsg(($state['status'] ?? '') === 'completed'
				? cfg_t('Проверка целостности завершена.', 'Integrity scan completed.')
				: cfg_t('Пакет проверен. Остальные файлы будут продолжены без повторного чтения.', 'Batch checked. Remaining files will resume without rereading completed entries.'));
		}
		elseif ($action === 'acknowledge')
		{
			...
			addMsg(cfg_t('Алерт принят к сведению. Эталон не изменён, предупреждение останется до восстановления файлов.', 'Alert acknowledged. The baseline was not changed; the warning remains until files are restored.'));
		}
	...
	catch (Throwable $exception)
	{
		...
		addMsg(cfg_t('Действие безопасности остановлено: ', 'Security action stopped: ') . $exception->getMessage());
	}
...
$securityFindingLabels = array(
	'modified' => cfg_t('Изменён', 'Modified'),
	'missing' => cfg_t('Отсутствует', 'Missing'),
	'unexpected' => cfg_t('Неожиданный', 'Unexpected'),
	'unexpected_link' => cfg_t('Неожиданная ссылка', 'Unexpected link'),
	'unexpected_executable' => cfg_t('Неожиданный исполняемый файл', 'Unexpected executable'),
	'customized' => cfg_t('Кастомизирован', 'Customized'),
	'unreadable' => cfg_t('Не читается', 'Unreadable'),
);
...
<section class="mx-auto max-w-6xl space-y-7">
	<header class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
		<div>
			<span class="mb-3 inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-blue-600 dark:text-blue-400"><i class="fa-solid fa-shield-halved" aria-hidden="true"></i><?php echo cfg_t('Конфигуратор', 'Configurator'); ?></span>
			<h1 class="text-3xl font-black text-brand dark:text-white sm:text-4xl"><?php echo cfg_t('Безопасность', 'Security'); ?></h1>
			<p class="mt-2 max-w-3xl text-sm font-medium text-gray-500 dark:text-gray-400"><?php echo cfg_t('Готовность production-среды и локальная проверка файлов по эталону точного официального релиза.', 'Production readiness and a local file check against the exact official release baseline.'); ?></p>
		</div>
		<form method="post" hx-boost="false">
			...
			<button type="submit" class="<?php echo $cfgButtonClass; ?> disabled:cursor-not-allowed disabled:opacity-50" <?php echo $securityScanner instanceof IntegrityScanner ? '' : 'disabled'; ?>><i class="fa-solid <?php echo $securityRunning ? 'fa-forward-step' : 'fa-magnifying-glass'; ?>" aria-hidden="true"></i><?php echo $securityRunning ? cfg_t('Продолжить проверку', 'Continue scan') : cfg_t('Проверить сейчас', 'Scan now'); ?></button>
		</form>
	</header>
	...
	<?php if ($securityScannerError !== '') { ?>
		<aside class="rounded-lg border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
			<strong class="font-extrabold"><?php echo $securityOfficialBuild || $securityBundledBaselinePresent
				? cfg_t('Эталон релиза не прошёл проверку.', 'The release baseline did not pass validation.')
				: cfg_t('Локальная сборка запущена без эталона релиза.', 'The local build is running without a release baseline.'); ?></strong>
			<p class="mt-2 text-xs font-medium opacity-80"><?php echo $securityOfficialBuild || $securityBundledBaselinePresent
				? cfg_t('Не создавайте новый эталон из текущих рабочих файлов. Повторно разверните точный официальный Docker image или shared-hosting архив этой версии.', 'Do not create a new baseline from the current live files. Redeploy the exact official Docker image or shared-hosting archive for this version.')
				: cfg_t('Для hs.local и checkout из исходников это штатно. Проверка станет доступна в официальном Docker image или shared-hosting архиве, где эталон создаётся во время сборки релиза.', 'This is expected for hs.local and source checkouts. The scan becomes available in the official Docker image or shared-hosting archive where the baseline is generated during the release build.'); ?></p>
		</aside>
	<?php } ?>
	...
	<?php if ($securityCritical) { ?>
		<aside role="alert" class="flex flex-col gap-4 rounded-lg border border-red-200 bg-red-50 p-5 text-red-950 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-100 sm:flex-row sm:items-center sm:justify-between">
			<div class="flex items-start gap-3"><i class="fa-solid fa-shield-virus mt-1" aria-hidden="true"></i><div><strong class="font-extrabold"><?php echo cfg_t('Обнаружены критические изменения: ', 'Critical file changes detected: '); ?><?php echo (int)$securityCounts['critical']; ?></strong><p class="mt-1 text-xs font-medium opacity-80"><?php echo cfg_t('Подтверждение не скрывает alert и не меняет эталон. Он исчезнет только после восстановления файлов и новой проверки.', 'Acknowledgement does not hide the alert or change the baseline. It clears only after files are restored and scanned again.'); ?></p></div></div>
			<?php if (!$securityAcknowledged) { ?><form method="post" hx-boost="false"><input type="hidden" name="csrf" value="<?php echo htmlspecialchars($securityCsrf, ENT_QUOTES, 'UTF-8'); ?>"><input type="hidden" name="securityAction" value="acknowledge"><button class="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-red-300 bg-white px-4 text-xs font-extrabold text-red-700 dark:border-red-500/40 dark:bg-black/20 dark:text-red-100" type="submit"><?php echo cfg_t('Принять к сведению', 'Acknowledge'); ?></button></form><?php } else { ?><span class="shrink-0 rounded-full bg-red-100 px-3 py-1.5 text-xs font-extrabold text-red-700 dark:bg-red-500/20 dark:text-red-100"><?php echo cfg_t('Принято к сведению', 'Acknowledged'); ?></span><?php } ?>
		</aside>
	<?php } ?>
	...
	<section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
		<?php
		$securityCards = array(
			array(cfg_t('Без изменений', 'Unchanged'), (int)$securityCounts['unchanged'], 'fa-circle-check', 'text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 dark:text-emerald-300'),
			array(cfg_t('Кастомизировано', 'Customized'), (int)$securityCounts['customized'], 'fa-palette', 'text-blue-600 bg-blue-50 dark:bg-blue-500/10 dark:text-blue-300'),
			array(cfg_t('Критические', 'Critical'), (int)$securityCounts['critical'], 'fa-triangle-exclamation', 'text-red-600 bg-red-50 dark:bg-red-500/10 dark:text-red-300'),
			array(cfg_t('Проверено', 'Checked'), (int)($securityState['checked'] ?? 0), 'fa-list-check', 'text-violet-600 bg-violet-50 dark:bg-violet-500/10 dark:text-violet-300'),
		);
	...
	<?php if ($securityRunning) { ?>
		<section class="rounded-lg border border-blue-200 bg-blue-50 p-5 dark:border-blue-500/30 dark:bg-blue-500/10"><div class="flex items-center justify-between gap-4 text-sm font-extrabold text-blue-900 dark:text-blue-100"><span><?php echo cfg_t('Проверка выполняется', 'Scan in progress'); ?></span><span><?php echo $securityPercent; ?>%</span></div><div class="mt-3 h-2 overflow-hidden rounded-full bg-blue-100 dark:bg-blue-950"><div class="h-full rounded-full bg-blue-600" style="width: <?php echo $securityPercent; ?>%"></div></div><p class="mt-3 text-xs font-medium text-blue-800 dark:text-blue-200"><?php echo (int)$securityState['checked']; ?> / <?php echo (int)$securityState['total']; ?>. <?php echo cfg_t('Один запуск ограничен по времени и числу файлов; cron продолжит с сохранённой позиции.', 'Each run is bounded by time and file count; cron resumes from the saved position.'); ?></p></section>
	<?php } ?>

	<section class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-[#151515]">
		<header class="flex flex-col gap-2 border-b border-gray-100 px-6 py-5 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-lg font-extrabold text-brand dark:text-white"><?php echo cfg_t('Целостность файлов', 'File integrity'); ?></h2><p class="mt-1 text-xs font-medium text-gray-500 dark:text-gray-400"><?php echo cfg_t('Twig-шаблоны считаются кастомизируемыми и сами по себе не вызывают alert.', 'Twig templates are customizable and never trigger an alert by themselves.'); ?></p></div><span class="text-xs font-bold text-gray-400"><?php echo cfg_t('Последняя проверка: ', 'Last scan: '); ?><?php echo $securityDate((int)($securityState['completed_at'] ?? 0)); ?></span></header>
		<?php if (empty($securityState['findings'])) { ?>
			<p class="p-8 text-center text-sm font-medium text-gray-500 dark:text-gray-400"><?php echo ($securityState['status'] ?? '') === 'completed' ? cfg_t('Отклонений от эталона нет.', 'No baseline deviations found.') : cfg_t('Запустите первую проверку.', 'Run the first scan.'); ?></p>
		<?php } else { ?>
			<div class="overflow-x-auto"><table class="w-full min-w-[850px] text-left text-sm"><thead class="bg-gray-50 text-[11px] font-extrabold uppercase tracking-wider text-gray-400 dark:bg-[#1A1A1A]"><tr><th class="px-5 py-3"><?php echo cfg_t('Файл', 'File'); ?></th><th class="px-5 py-3"><?php echo cfg_t('Статус', 'Status'); ?></th><th class="px-5 py-3"><?php echo cfg_t('Размер: эталон / факт', 'Size: expected / actual'); ?></th><th class="px-5 py-3"><?php echo cfg_t('Изменён', 'Modified'); ?></th><th class="px-5 py-3"><?php echo cfg_t('Проверен', 'Checked'); ?></th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-gray-800">
			<?php foreach ((array)$securityState['findings'] as $finding) { $neutral = ($finding['severity'] ?? '') === 'neutral'; ?>
				<tr><td class="max-w-md break-all px-5 py-4 font-mono text-xs text-brand dark:text-gray-100"><?php echo htmlspecialchars((string)$finding['path'], ENT_QUOTES, 'UTF-8'); ?><span class="mt-1 block font-sans text-[11px] font-bold text-gray-400"><?php echo ($finding['class'] ?? '') === 'customizable' ? cfg_t('Кастомизируемый', 'Customizable') : cfg_t('Ядро', 'Core'); ?></span></td><td class="px-5 py-4"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-extrabold <?php echo $neutral ? 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300' : 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-300'; ?>"><?php echo htmlspecialchars($securityFindingLabels[$finding['status']] ?? (string)$finding['status'], ENT_QUOTES, 'UTF-8'); ?></span></td><td class="px-5 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400"><?php echo htmlspecialchars($securitySize($finding['expected_size'] ?? null) . ' / ' . $securitySize($finding['actual_size'] ?? null), ENT_QUOTES, 'UTF-8'); ?></td><td class="px-5 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400"><?php echo $securityDate((int)($finding['mtime'] ?? 0)); ?></td><td class="px-5 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400"><?php echo $securityDate((int)($finding['checked_at'] ?? 0)); ?></td></tr>
			<?php } ?>
			</tbody></table></div>
			<?php if (!empty($securityState['findings_truncated'])) { ?><p class="border-t border-amber-100 bg-amber-50 px-5 py-3 text-xs font-bold text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200"><?php echo cfg_t('Список ограничен. Счётчики выше учитывают все найденные отклонения.', 'The list is bounded. Counters above include every detected deviation.'); ?></p><?php } ?>
		<?php } ?>
	</section>
	...
	<details class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-[#151515]" <?php echo $securityPreflight['blockers'] ? 'open' : ''; ?>>
		<summary class="cursor-pointer list-none px-6 py-5"><div class="flex items-center justify-between gap-4"><div><h2 class="text-lg font-extrabold text-brand dark:text-white"><?php echo cfg_t('Production preflight', 'Production preflight'); ?></h2><p class="mt-1 text-xs font-medium text-gray-500 dark:text-gray-400"><?php echo cfg_t('Блокирующие условия проверяются повторно перед бэкапом и обновлением.', 'Blocking conditions are checked again before backup and update.'); ?></p></div><div class="flex shrink-0 items-center gap-3"><span class="rounded-full px-3 py-1 text-xs font-extrabold <?php echo $securityPreflight['blockers'] ? 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-300' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300'; ?>"><?php echo $securityPreflight['blockers'] ? count($securityPreflight['blockers']) . ' ' . cfg_t('блокеров', 'blockers') : cfg_t('Готово', 'Ready'); ?></span><i class="fa-solid fa-chevron-down text-xs text-gray-400" aria-hidden="true"></i></div></div></summary>
